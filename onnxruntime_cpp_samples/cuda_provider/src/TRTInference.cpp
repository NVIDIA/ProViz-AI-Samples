...
TRTInference::TRTInference(std::string model_path,
                           ...
                           uint32_t device_id, bool use_fp16,
                           cudaStream_t stream,
                           std::unique_ptr<Ort::SessionOptions> opts)
    : NVInference(std::move(opts)) {
  nvtxRangePush("TRT EP creation");

  CheckCUDA(cudaSetDevice(device_id));
  const char *name = "trt_inference";
  ...
  /// setup trt options
  {
    m_trt_options = std::make_unique<OrtTensorRTProviderOptions>();
    m_trt_options->device_id = int(device_id);
    m_trt_options->has_user_compute_stream = 1;
    m_trt_options->trt_max_partition_iterations = 1000;
    m_trt_options->trt_min_subgraph_size = 1;
    m_trt_options->user_compute_stream = stream;
    m_trt_options->trt_fp16_enable = use_fp16;
    m_trt_options->trt_int8_calibration_table_name = "";
    m_trt_options->trt_engine_cache_enable = true;
    m_trt_options->trt_engine_cache_path = "./";
    size_t mem_free = 0, mem_tot = 0;
    cudaMemGetInfo(&mem_free, &mem_tot);
    m_trt_options->trt_max_workspace_size = mem_free / (1024 * 1024);
    if (m_trt_options->user_compute_stream != nullptr) {
      m_compute_stream =
          static_cast<cudaStream_t>(m_trt_options->user_compute_stream);
    } else {
      CheckCUDA(cudaStreamCreate(&m_compute_stream));
      m_trt_options->user_compute_stream = m_compute_stream;
    }

    m_session_options->AppendExecutionProvider_TensorRT(*m_trt_options);
  }
...
void TRTInference::runAsync(Ort::Value *input, Ort::RunOptions *run_options) {
  nvtxRangePush("TensorRT EP runAsync");
